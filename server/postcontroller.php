...
// Start of deleteAjax
// Delete post by id
// Link test: http://localhost/dealcongnghe/server/postcontroller?action=deleteAjax&Id=1
...
function updateAjax($postId, $newPost){
     ...
     //3. Response result to client/user
     if ($conn->query($sql) === TRUE) {
     echo "Product with Id $postId has been successfully updated";
     } else {
     echo "Error: " . $sql . "<br>" . $conn->error;
     }
...
// Start of manageAjax
// Manage post
// Link test: http://localhost/dealcongnghe/server/postcontroller?action=manageAjax
function manageAjax(){
     //Get data from database
     $servername = "localhost";
     $username = "root";
